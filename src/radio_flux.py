...
def plot_and_save_spectrum(frequency, flux, survey, predict, output_filename='radio_spectrum.png'):
    """
    ...
    for i, (x, y, survey) in enumerate(zip(frequency, flux, survey)):
            plt.annotate(f'({x:.2f}, {y:.2f}, {survey:s})', (x, y), textcoords="offset points", xytext=(0,10), ha='center')
    ...
    plt.xlabel('Restframe Frequency [GHz]')
    plt.ylabel('Flux Density [muJy]')
    plt.legend()
...
def calc_pred_flux(frequency, flux, survey, f_pred, slope=None):
    """
    ...
    plot_and_save_spectrum(frequency, flux, survey, [x_predict, y_predict], output_filename='radio_spectrum.png')
